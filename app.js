...
const CJK_CHAR_REGEX = /[\u3400-\u4DBF\u4E00-\u9FFF\uF900-\uFAFF\u3040-\u30FF\uAC00-\uD7AF]/g;
...
function estimateTokensFromText(text, langHint) {
  const content = typeof text === 'string' ? text.trim() : '';
  if (!content) {
    return 0;
  }
  const withoutSpaces = content.replace(/\s+/g, '');
  const totalLength = withoutSpaces.length;
  if (!totalLength) {
    return 0;
  }
  const cjkMatches = withoutSpaces.match(CJK_CHAR_REGEX);
  const cjkCount = cjkMatches ? cjkMatches.length : 0;
  const nonCjkCount = Math.max(0, totalLength - cjkCount);
  let estimate = 0;
  if (cjkCount) {
    estimate += cjkCount * 0.5;
  }
  if (nonCjkCount) {
    estimate += nonCjkCount / 4;
  }
  if (!estimate) {
    estimate = totalLength / 4;
  }
  if (langHint === 'en' && nonCjkCount) {
    estimate = Math.max(estimate, totalLength / 4);
  }
  if (langHint === 'zh' && cjkCount && !nonCjkCount) {
    estimate = Math.max(estimate, cjkCount * 0.5);
  }
  return Math.min(TOKEN_LIMIT, Math.max(0, Math.round(estimate)));
}

function updateTextEstimates() {
  if (state.calc.mode !== 'text') {
    return;
  }
  const scenario = state.calc.currentScenario;
  const lang = scenario.lang || 'zh';
  const promptText = state.calc.textSamples.prompt || scenario.textSamples?.prompt || '';
  const completionText = state.calc.textSamples.completion || scenario.textSamples?.completion || '';
  const promptTokens = estimateTokensFromText(promptText, lang);
  const completionTokens = estimateTokensFromText(completionText, lang);
